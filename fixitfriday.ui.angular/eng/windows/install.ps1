...
param(
  [string]
  $InstallPath = "c:\Ed-Fi\Fix-it-Friday\Angular",
...
function Install-NginxService {
  ...
  $fixItFridayUiExe = "$InstallPath\$winSwVersion\FixItFridayUiAngular.exe"
  ...
  $xmlFile = "$InstallPath\$winSwVersion\FixItFridayUiAngular.xml"
  Copy-Item -Path "$PSScriptRoot\FixItFridayUiAngular.xml" -Destination $xmlFile -Force
