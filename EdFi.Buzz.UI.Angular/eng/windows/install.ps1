...
param(
  [string]
  $InstallPath = "c:\Ed-Fi\Buzz\Angular",
...
function Install-NginxService {
  ...
  $xmlFile = "$InstallPath\$winSwVersion\EdFiBuzzUiAngular.xml"
  Copy-Item -Path "$PSScriptRoot\EdFiBuzzUiAngular.xml" -Destination $xmlFile -Force
