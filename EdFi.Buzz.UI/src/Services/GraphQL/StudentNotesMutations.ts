// SPDX-License-Identifier: Apache-2.0
// Licensed to the Ed-Fi Alliance under one or more agreements.
// The Ed-Fi Alliance licenses this file to you under the Apache License, Version 2.0.
// See the LICENSE and NOTICES files in the project root for more information.


import gql from 'graphql-tag';

const addStudentNote = gql`
mutation ($staffkey: Int!, $studentschoolkey: String!, $note: String!) {
  addstudentnote(staffkey: $staffkey, studentschoolkey: $studentschoolkey, note: $note){
    studentnotekey
    dateadded
  }
}
`;

const deleteStudentNote = gql`
mutation ($staffkey: Int!, $studentnotekey: Int!) {
  deletestudentnote(staffkey: $staffkey, studentnotekey: $studentnotekey){
    studentnotekey
  }
}
`;

export { addStudentNote, deleteStudentNote };

